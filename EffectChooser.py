...
class EffectChooser:
    ...
    def __init__(self, pixel_buffer=None, pixel_buffer_list=None):
        ...
        """
        assert (
            pixel_buffer is None or pixel_buffer_list is None
        ), "Specify buffer or buffer_list, not both"
    ...
    def get_chosen_effects(self, effect_cmd):
        ...
        print(
            "Name:", effect_name, "Comp:", comp_name, "Color:", color, "Speed:", speed
        )

        if effect_name == "wipe" and comp_name in topo_comps:
            return [
                WipeFillEffect(self._pixel_buffer, color=color, slowness=speed),
                WaitEffect(self._pixel_buffer, slowness=speed),
            ]
            ...
            if comp_name in topo_comps:
                return [
                    SqueezeFillEffect(self._pixel_buffer, color=color, slowness=speed)
                ]
            elif comp_name in div_names:
                divs = int(comp_name)
                return [
                    SqueezeFillEffect(
                        self._pixel_buffer_list[i], color=color, slowness=speed
                    )
                    for i in range(divs)
                ]
            ...
            if comp_name in topo_comps:
                return [
                    FlashingLightsEffect(
                        self._pixel_buffer, color=color, slowness=speed
                    )
                ]
            elif comp_name in div_names:
                divs = int(comp_name)
            ...
            if comp_name in topo_comps:
                return [
                    RainbowColorEffect(self._pixel_buffer, color=color, slowness=speed)
                ]
            elif comp_name in div_names:
                divs = int(comp_name)
                return [
            ...
            if comp_name in topo_comps:
                return [
                    SoundMeterEffect(self._pixel_buffer, color=color, slowness=speed)
                ]
            ...
            return [
                ...
                WipeFillEffect(
                    self._pixel_buffer_list[6], color=BUTTERSCOTCH, slowness=5
                ),
                ...
                WipeFillEffect(
                    self._pixel_buffer_list[10], color=BUTTERSCOTCH, slowness=5
                ),
            ...
            if comp_name in topo_comps:
                ...
                divs = int(comp_name)
                return [
                    RainbowEffect(self._pixel_buffer_list[i], slowness=speed)
                    for i in range(divs)
                ]
            ...
            if comp_name in topo_comps:
                ...
                divs = int(comp_name)
                return [
                    DripEffect(self._pixel_buffer_list[i], slowness=1, tap=speed)
                    for i in range(divs)
                ]
            ...
            if comp_name == "frameNcorners":
                re = [
                    RunnerEffect(
                        self._pixel_buffer_list[5], color=color, slowness=speed
                    )
                ]
                return re + [
                    FlipFlopEffect(
                        self._pixel_buffer_list[i], color=red, slowness=speed
                    )
                    for i in range(4)
                ]
            ...
            if comp_name in topo_comps:
                ...
                return ges + we
        elif effect_name == "shoot":
            div_names = ValidDivisions
            if comp_name in topo_comps:
                return [shootEffect(self._pixel_buffer, color=color, slowness=speed)]
            elif comp_name in div_names:
                divs = int(comp_name)
                return [
                    shootEffect(self._pixel_buffer_list[i], color=color, slowness=speed)
                    for i in range(divs)
                ]

        else:
            print(
                f"Command [{effect_cmd}] matches nothing.  Just gonna do a purple wipe"
            )
